...
@interface NationInsureViewController ()<WKNavigationDelegate,WKUIDelegate,WKScriptMessageHandler,UINavigationControllerDelegate,UINavigationBarDelegate>
...
@implementation NationInsureViewController
...
- (void)webViewloadURLType{
    switch (self.loadType) {
        case loadWebURLString:{
            ...
            //加载网页
           [request setValue:@"http://nglwx.gwcslife.com" forHTTPHeaderField:@"Referer"];
...
//    UIButton *backButton = [self navigationButtonWithImage:[UIImage imageNamed:@"navi_back"] highligthtedImage:nil action:@selector(customBackItemClicked)];
//    UIButton *closeButton = [self navigationButtonWithTitle:@"关闭" action:@selector(closeItemClicked)];
...
- (WKWebView *)wkWebView{
    if (!_wkWebView) {
        ...
        _wkWebView = [[WKWebView alloc] initWithFrame:CGRectMake(0,-64, kScreenWidth, kScreenHeight) configuration:Configuration];
