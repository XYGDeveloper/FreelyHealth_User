...
- (instancetype)initWithTitle:(NSString *)title attributeMessage:(NSMutableAttributedString *)attrMessage delegate:(id<YQAlertViewDelegate>)delegate buttonTitles:(NSString *)buttonTitles, ...{
    ...
    [yqAlert.attrMessage addAttribute:NSFontAttributeName value:FontNameAndSize(15) range:NSMakeRange(0, attrMessage.length)];
...
- (void)SetupSubviews
{
    ...
    titleLabel.font = FontNameAndSize(18);
    ...
    messageLabel.font  = FontNameAndSize(14);
...
- (void)layoutSubviews{
    ...
    if (self.buttonTitleArray.count > 0) {
        ...
        for (NSString *buttonTitle in _buttonTitleArray) {
            ...
            if ([buttonTitle isEqualToString:@"确定"] || [buttonTitle isEqualToString:@"我已阅读并同意"] || [buttonTitle isEqualToString:@"去加入"]) {
