...
- (void)configLayout {
    ...
    [self.emptyLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.emptyImgView.mas_bottom).offset(15);
        make.left.right.equalTo(@0);
    }];
    
    [self.operationButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.emptyLabel.mas_bottom).offset(33);
        make.centerX.equalTo(self);
...
- (UILabel *)emptyLabel {
    if (!_emptyLabel) {
        ...
        _emptyLabel.font = FontNameAndSize(16);
        _emptyLabel.numberOfLines  = 0;
        _emptyLabel.textColor = DefaultGrayLightTextClor;
