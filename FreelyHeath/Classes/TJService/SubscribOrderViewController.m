...
#import "SubsOrderTableViewCell.h"
...
@interface SubscribOrderViewController ()<UIAlertViewDelegate,UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate,ApiRequestDelegate,selectPayModelDelegate,BaseMessageViewDelegate>
{
...
@implementation SubscribOrderViewController
...
- (void)viewDidLoad {
    ...
    [self.OrderdetailTableView registerClass:[SubsOrderTableViewCell class] forCellReuseIdentifier:NSStringFromClass([SubsOrderTableViewCell class])];
    [self.OrderdetailTableView registerClass:[SubsOrderHaveServiceTableViewCell class] forCellReuseIdentifier:NSStringFromClass([SubsOrderHaveServiceTableViewCell class])];
        ...
        if (self.hzDetail == YES) {
            ...
            if ([model.status isEqualToString:@"1"]) {
                ...
                if ([model.nojg isEqualToString:@"0"]) {
                    [self.toSub setTitle:@"联系客服" forState:UIControlStateNormal];
                    [self.view addSubview:self.toSub];
                    [self.toSub mas_makeConstraints:^(MASConstraintMaker *make) {
                        make.left.right.bottom.mas_equalTo(0);
                        make.height.mas_equalTo(50);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        cell.detail = ^{
            if (self.hzDetail == YES) {
              //此页面跳转H5
                ...
                detail.title = self.model.name;
                detail.id = self.model.taocanid;
        ...
        if ([self.model.fuwufei isEqualToString:@"1"]) {
            SubsOrderHaveServiceTableViewCell *cell =[tableView dequeueReusableCellWithIdentifier:NSStringFromClass([SubsOrderHaveServiceTableViewCell class])];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            [cell refreshWithModel:self.model];
            return cell;
        }else{
            SubsOrderTableViewCell *cell =[tableView dequeueReusableCellWithIdentifier:NSStringFromClass([SubsOrderTableViewCell class])];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            [cell refreshWithModel:self.model];
            return cell;
        }
